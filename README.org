...
Mold only has a few top-level components,
 ...
 - ~signal~ are formulae that evaluate to specified event
   ...
   end of the tick. An unchanged signal is not dispatched again, so an
   event held by a signal is emitted only when the signal's value
   changes.
...
Mold has the following builtin types:
 - Int (3, -6)
 - Real (3.14, -273.15)
 - Bool (true, false)
 - String ("Hello, world!")
...
which permits =null= value. Most operations propagate =null=, and the
coalescing operator can provide a fallback:
...
#+BEGIN_SRC text
  ...
  Temp = 15.3
  Temp = 15.6
  (!) 21.1℃ -- 2026-09-18 05:51:04.111135633
  Opened AC
  Temp = 21.1
  (!) 26.3℃ -- 2026-09-18 05:51:04.211540186
  Temp = 26.3
  (!) 32.2℃ -- 2026-09-18 05:51:04.311828508
  Temp = 32.2
  Closed AC
  Temp = 25.5
#+END_SRC

For a full documentation of the language, see the [[file:doc/00-home.org][doc/]]
folder. Alternatively, visit [[https://pancakey8.github.io/mold/][the website]]
